...
def inspect_document_for_cybersecurity(link):
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            time.sleep(REQUEST_INTERVAL)
            if response.status == 200:
                document_text = response.read().decode('utf-8')  # Keep the document text as is, respecting case
...
def fetch_filings_from_rss(url):
    ...
    try:
        ...
        if e.code == 403:
            logger.critical(f"Error fetching filings: HTTP Error 403: Forbidden. Access to {url} is denied.")
            exit()
        else:
